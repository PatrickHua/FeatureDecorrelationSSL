    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                #     for j in range(W):
                #         w = m.weight[:,:,i,j]
                #         m.weight.data[:,:,i,j] = (lambda U,_,V: U.mm(V.t()))(*w.svd())
                # breakpoint()
                # print(m.weight.shape)
                # m.weight.data = (lambda U,_,V: U.mm(V.t()))(*m.weight.flatten(1).svd()).view_as(m.weight.data)
...
if __name__ == "__main__":
    ...
    # out_features = 30
    image_size = 32
    # X = torch.stack([torch.empty(in_features).normal_(mean=0, std=1) for _ in range(batch_size)])
    X = torch.randn(batch_size, 3, image_size, image_size)
    # breakpoint()
    # print(corrcoef(X.permute(0,2,3,1).flatten(1, -1)).mean())
    print(corrcoef(X[:,:,0,0]).mean())
    with torch.no_grad():
        Y = model(X)
    # print(Y.shape)
    print(corrcoef(Y).mean())
