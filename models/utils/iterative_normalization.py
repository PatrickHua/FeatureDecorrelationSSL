...
class iterative_normalization_py(torch.autograd.Function):
    ...
    def forward(ctx, *args, **kwargs):
        ...
        if training:
            ...
            # Sigma = torch.baddbmm(beta=eps, input=P[0], alpha=1. / m, batch1=xc, batch2=xc.transpose(1, 2))
            # baddbmm(Number beta=eps, Tensor input, Number alpha, Tensor batch1, Tensor batch2, *, Tensor out)

            Sigma = torch.baddbmm(input=P[0], batch1=xc, batch2=xc.transpose(1,2), beta=eps, alpha=1/m)
            # baddbmm(Tensor input, Tensor batch1, Tensor batch2, *, Number beta, Number alpha, Tensor out)
            ...
            running_wmat = momentum * wm + (1. - momentum) * running_wmat
        else:
            xc = x - running_mean
            wm = running_wmat
        xn = wm.matmul(xc)
    ...
    def backward(ctx, *grad_outputs):
        ...
        for k in range(ctx.T, 1, -1):
            ...
            # baddbmm_(Number beta, Number alpha, Tensor batch1, Tensor batch2)
            # baddbmm_(Tensor batch1, Tensor batch2, *, Number beta, Number alpha)
            # g_P.baddbmm_(g_tmp, )
